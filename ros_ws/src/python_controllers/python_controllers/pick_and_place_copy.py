...
# ---------------------------------------------------------
       ...
       # keep PLACE_1 as-is (works in lab)
        PLACE_1 = [PLACE_BASE, 0.2715, -1.1137, -1.0324, PLACE_TILT, GRIPPER_CLOSED]

        # small incremental flex adjustments for each extra block
        DELTA = 0.10  # ~6 deg; tune in the lab

        def raise_one_block(prev_place, sign=1.0):
            base, q2, q3, q4, q5, grip = prev_place
            dq3 = -sign * DELTA   # make elbow slightly less bent
            dq4 = +sign * DELTA   # bend wrist a bit more to keep pitch
            return [base, q2, q3 + dq3, q4 + dq4, q5, grip]

        PLACE_2 = raise_one_block(PLACE_1)
        PLACE_3 = raise_one_block(PLACE_2)
        PLACE_4 = raise_one_block(PLACE_3)
        PLACE_5 = raise_one_block(PLACE_4)
